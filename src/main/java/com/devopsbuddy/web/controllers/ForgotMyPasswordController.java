...
public class ForgotMyPasswordController {
    ...
    @Value("${webmaster.email}") // ~/.devopsbuddy/application-common.properties file
    ...
    public String forgotPasswordPost(HttpServletRequest request,
        ...
        // if there isn't a user registered with that email, log a warning message to console
        if(null == passwordResetToken){
            LOG.warn("Couldn't find a password reset token for email {}", email);
        // otherwise, just log the token to make sure everything works as expected
        } else{
            ...
            mailMessage.setSubject("\"[Devosbuddy]: How to Reset Your Password\"");
    ...
    // handle get request to the forgotpassword link user get via email
    // returns the view name corresponding to the boostrap page where
    ...
    public String changeUserPasswordGet(@RequestParam("id") long id,
                                        ...
                                        Locale locale,
                                        ModelMap model){
        // StringUtils.isEmpty(token) returns true if null or empty
        ...
        // OK to proceed. We auto-authenticate the user to that in the POST request we can if the user
        // is authenticated

        // we want to make sure that an authenticated can perform this action
