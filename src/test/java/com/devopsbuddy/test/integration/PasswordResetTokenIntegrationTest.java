...
public class PasswordResetTokenIntegrationTest extends AbstractIntegrationTest {
    ...
    // making sure that repository works too
    @Test
    public void testTokenExpirationLength() throws Exception{
        ...
        // expiration from value defined earlier
        ...
        // we verify if its the same that was created in the db
        LocalDateTime actualTime = passwordResetToken.getExpiryDate();
        Assert.assertNotNull(actualTime);
        Assert.assertEquals(expectedTime, actualTime);
    }

    // we want to test that we can find a single record by token value
    @Test
    ...
    // we want to make sure that we can delete a token via primary key
    @Test
    ...
    // we want to make sure that if we delete a user record,
    // all the password reset token records in db get deleted too
    // works because of attribute (one to many) passwordResetTokens in User entity
    @Test
